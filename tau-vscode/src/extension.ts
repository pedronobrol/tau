...
import { ProofCacheManager } from './proofCache';

let decorationProvider: DecorationProvider;
let diagnosticsManager: DiagnosticsManager;
let tauClient: TauClient;
let completionProvider: CompletionProvider;
let proofCache: ProofCacheManager;
let reactiveUpdateTimeout: NodeJS.Timeout | undefined;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    proofCache = new ProofCacheManager();
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tau.viewCachedSpecs', async (document: vscode.TextDocument, line: number, cachedInfo: any) => {
            await viewCachedSpecs(document, line, cachedInfo, codeLensProvider);
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('tau.clearCache', async () => {
            proofCache.clear();
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tau.showProofDetails', async (document: vscode.TextDocument, line: number) => {
 ...
 */
function reactivelyUpdateVerificationStatus(document: vscode.TextDocument, codeLensProvider: CodeLensProvider) {
    const editor = vscode.window.activeTextEditor;
    ...
    const config = vscode.workspace.getConfiguration('tau');
    const displayMode = config.get<string>('statusDisplayMode', 'codelens');
 ...
 */
async function checkCachedSpecsByBody(
    document: vscode.TextDocument,
    line: number,
    funcInfo: { name: string; source: string; requires: string; ensures: string; invariants: string[]; variant: string },
 ...
 */
async function autoCheckCachedProofs(document: vscode.TextDocument, codeLensProvider: CodeLensProvider) {
    const editor = vscode.window.activeTextEditor;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line === '@safe' || line === '@safe_auto') {
            ...
            try {
                const proofCheck = await tauClient.checkProof(
                    funcInfo.name,
                    funcInfo.source,
                    funcInfo.requires,
                    funcInfo.ensures,
...
async function verifyFunction(document: vscode.TextDocument, line: number, codeLensProvider: CodeLensProvider) {
    ...
    // Check server cache (if not in local cache)
    try {
        let proofCheck = await tauClient.checkProof(
            funcInfo.name,
            funcInfo.source,
            funcInfo.requires,
            funcInfo.ensures,
 ...
 */
function extractFunctionInfo(document: vscode.TextDocument, line: number): {
    name: string;
    source: string;
    requires: string;
    ensures: string;
 ...
 */
async function viewCachedSpecs(
    document: vscode.TextDocument,
    line: number,
    cachedInfo: { count: number; specs: any; hash: string; verified: boolean },
    codeLensProvider: CodeLensProvider
) {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document !== document) {
...
function formatSpecs(specs: any): string {
    ...
    if (specs.invariants && specs.invariants.length > 0) {
        specs.invariants.forEach((inv: string) => {
 ...
 */
async function replaceFunctionSpecs(
    document: vscode.TextDocument,
    safeLine: number,
    newSpecs: { requires: string; ensures: string; invariants: string[]; variant: string }
) {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document !== document) {
        return;
    ...
    await editor.edit(editBuilder => {
        const range = new vscode.Range(
            new vscode.Position(specStartLine, 0),
            new vscode.Position(specEndLine, 0)
        );
 ...
 */
async function showProofDetailsPanel(
    document: vscode.TextDocument,
    line: number,
    codeLensProvider: CodeLensProvider,
    ...
    const panel = vscode.window.createWebviewPanel(
        'tauProofDetails',
        `Proof Details: ${status.functionName || funcInfo?.name || 'Function'}`,
            ...
            <div class="section">
                ...
                <div class="spec-item">
                    ...
                    <div class="spec-value">
                        ${status.specs.invariants.map((inv: string) => `<code>${escapeHtml(inv)}</code>`).join('<br>')}
                    </div>
                </div>
                ` : ''}
